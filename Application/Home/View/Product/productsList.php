    ...
    <div class="thewidth100 bg_gray clearfix" > 
            <div class="thewidth" id="p">
                <div class="width font-12 c666">
                    <a href="{:U('Home/index')}">首页 </a>><?php echo $cate_info['category_name']; ?>
                </div>
                <!-- list -->
                <div class="protop protoplist mt10">
                    <div class="width yxz">
                        ...
                        <div class="wb10 f_r"><a href="<?php echo U('Product/productsList/order_param/0?cid='.$cid.'#p');?>" class="qcall f_r">清除全部<span></span></a></div>
                    </div>
                    <ul>
                        <li>
                    ...
                    <?php foreach ($cate_attribute as $k => $vo): ?>
                        <li>
                            <div class="pro-bg c999">{$vo.attribute_name}：</div>
                            <div class="wb90 f_l pro-lb">
                            <?php   
                                $str = '';
                                $url = '';
                                ...
                                $url = U('Product/productsList/param/'.$str.'/order_param/'.$order_param.'?cid='.$cid.'#p');
                            ?>
                                <a href="<?php echo $url;?>"  <?php if($param[$k+3] == 0){echo 'class="current8"';} ?> >不限</a>
                                <?php foreach ($vo['value'] as $key => $value): ?>
                                    <?php   
                                        $str = '';
                                        $url = '';
                                        for($i=0;$i<$total;$i++){
                                            if($i==($k+3)){
                                                $str.=$value['item_category_attribute_value_id'].'-';
                                            }else{
                                                $str.=$param[$i].'-';
                                            }
                                        }
                                        ...
                                        $url = U('Product/productsList/param/'.$str.'/order_param/'.$order_param.'?cid='.$cid.'#p');
                                    ?>
                                    <a href="<?php echo $url;?>"  <?php if($param[$k+3] == $value['item_category_attribute_value_id']){echo 'class="current8"';} ?>><?php echo $value['attribute_value'] ;?></a>
                                <?php endforeach; ?>
                ...
                <div class="screen mt10">
                    <ul>
                        <li><a href="<?php echo U('Product/productsList/param/'.implode('-', $param).'/order_param/0/cid/'.$cid);?>">默认排序</a></li>

                        <li><a  <?php $tmp_order_num=1; if($order_param==1){echo 'class="current12"'; $tmp_order_num=2;}else if($order_param==2){echo 'class="current13"'; $tmp_order_num=1;} ?>  href="<?php echo U('Product/productsList/param/'.implode('-', $param).'/order_param/'.$tmp_order_num.'/cid/'.$cid);?>"  
                         >最新</a></li>

                         <li><a  <?php $tmp_order_num=3; if($order_param==3){echo 'class="current12"'; $tmp_order_num=4;}else if($order_param==4){echo 'class="current13"'; $tmp_order_num=3;} ?>  href="<?php echo U('Product/productsList/param/'.implode('-', $param).'/order_param/'.$tmp_order_num.'/cid/'.$cid);?>"  
                         >销量</a></li>

                         <li><a  <?php $tmp_order_num=5; if($order_param==5){echo 'class="current12"'; $tmp_order_num=6;}else if($order_param==6){echo 'class="current13"'; $tmp_order_num=5;} ?>  href="<?php echo U('Product/productsList/param/'.implode('-', $param).'/order_param/'.$tmp_order_num.'/cid/'.$cid);?>"  
                         >价格</a></li>
                        ...
                        ?>
                            ...
                            $(document).ready(function(){
                                $("#price_form").click(function(e){
                                    ...
                                    window.location.href="__URL__/productsList/param/"+str+"/order_param/"+"<?php echo $order_param; ?>"+"?cid="+<?php echo $cid; ?>+"#p";
                                });
