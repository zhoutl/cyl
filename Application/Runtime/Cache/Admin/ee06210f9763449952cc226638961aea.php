<?php if (!defined('THINK_PATH')) exit(); W('Admin/head',array(array('admin_user_id'=>$admin_user_id)));?>
	...
	<script type="text/javascript" charset="utf-8" src="/cyl/Public/Common/ueditor/ueditor.config.js"></script>
	<script type="text/javascript" charset="utf-8" src="/cyl/Public/Common/ueditor/ueditor.all.min.js"> </script>
	<!--建议手动加在语言，避免在ie下有时因为加载语言失败导致编辑器加载失败-->
	<!--这里加载的语言文件会覆盖你在配置项目里添加的语言类型，比如你在配置项目里配置的是英文，这里加载的中文，那最后就是中文-->
	<script type="text/javascript" charset="utf-8" src="/cyl/Public/Common/ueditor/lang/zh-cn/zh-cn.js"></script>
...
<!---------------[ TITLE ]--------------->
                ...
                <div class="f_l">
                    <h4><i class="icon-bar-chart"></i> &nbsp; <span id="page_title">新增文章</span></h4>
                </div>
                <div class="f_r">
...
<!---------------[ /TITLE ]--------------->
            ...
            <div class="list_box">
                ...
                <div class="ja_add_from mb20">
                	<div class="add_from_head">新增文章</div>
                	<div class="add_from_body">
                    <form action="<?php echo U('Article/addArticle'); ?>" method="post" id="adminForm">
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="add_from_table">
                      <tr>
                        <td align="left" valign="top" class="aft_l">文章标题</td>
                        <td align="left" valign="middle" class="aft_r"><input type="text" name="title" class="form-control the_text3 verify_text"  placeholder="请输入文章标题" errormsg="文章标题为1-50个字" nullmsg="请输入文章标题" datatype="*1-50"></td>
                      </tr>
<!--                       <tr>
                        ...
                        <td align="left" valign="middle" class="aft_r">
							<select class="form-control the_text3 verify_text" name="article_category_id" errormsg="请选择所属分类" nullmsg="请选择所属分类" datatype="/^[1-9][0-9]{0,}$/">
								...
								<?php foreach($data['category_list'] as $k=>$v){ ?>
									<option value="<?php echo $v['article_category_id']; ?>"><?php echo $v['article_category_name']; ?></option>
								<?php } ?>
                            </select>
						</td>
                      </tr> -->
